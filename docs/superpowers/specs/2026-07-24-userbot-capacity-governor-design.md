...
**Статус:** approved with explicit YAGNI constraint; implementation plan may be written
...
7. Точные лимиты Telegram не считаются известными константами. В первой версии
   действует консервативный конфигурационный лимит; история собирается для
   последующей ручной проверки, но не меняет лимит автоматически.

### 2.1 Минимальный scope первой версии

Первая версия включает только:

1. учёт Telegram RPC и обработку всех FloodWait;
2. governor с cooldown, превентивным throttling и плавным recovery;
3. bounded slices и немедленную остановку по state;
4. geo eligibility и простую адаптивную частоту по empty streak;
5. capacity recommendation, Telegram alerts и компактный блок dashboard.

В первой версии нет новой PostgreSQL-таблицы, ML/самообучения лимитов,
ручных управляющих кнопок, отдельного scheduler-сервиса и event-driven
обработки Telegram updates.
...
- Новая PostgreSQL-миграция для operational metrics.
- Отдельный процесс или распределённый scheduler.
- Автоматическое обучение safe limit в первой версии.
...
|---|---|---:|---:|
| A Realtime | последний poll вернул сообщения | 2 минуты | ≤5 минут |
| B Active | после активности, 3–9 пустых polls | 5 минут | ≤10 минут |
| C Standard | новый chat или 10–29 пустых polls | 15 минут | ≤30 минут |
| D Quiet | редкие сообщения | 60 минут | ≤2 часов |
| E Dormant | длительно пустые, но eligible | 360 минут | ≤12 часов |
...
Bot API sender.
Для shadow-режима effective state/power хранятся отдельно от
recommended state/power: при выключенном enforcement превентивные решения
видны в метриках, но не меняют нагрузку. Реальный FloodWait/circuit всегда
меняет effective state и блокирует RPC.

### 7.3 Adaptive Scheduler внутри текущего worker

Scheduler выбирает только eligible chats и работает по `next_poll_at`.
Новый процесс или отдельный scheduler-сервис не создаётся.

- Основной порядок — earliest deadline first.
- При одинаковом deadline выше более просроченный чат класса A/B.
- Один чат одновременно имеет не более одного active poll lease.
- После poll scheduler обновляет простой empty streak и следующий срок.
...
- Компактный Redis hash `poll:summary:v1` хранит только агрегаты
  eligible/parked/quarantined, классы A–E и assigned count по аккаунтам для
  dashboard; API не обходит весь schedule.
...
`usable_daily_capacity` включает 30% reserve. В первой версии safe limit —
конфигурируемые 4000 RPC/сутки на аккаунт. Изменять его вверх до накопления
истории и отдельного решения владельца запрещено.

### 7.5 Metrics Store

Redis/AOF хранит minute/hour/day buckets, governor state и последние события
с TTL до 30 дней. Отдельная PostgreSQL-таблица не создаётся. Никакие phone,
api_hash, session contents или тексты сообщений в metrics не сохраняются.
...
Dashboard обновляется каждые 30 секунд и содержит fleet summary, карточки
аккаунтов и один график RPC. Управляющие кнопки start/stop, reset circuit и
изменение лимитов не входят в первую версию.
...
## 9. Простая адаптивная частота

В первой версии нет сложного priority score или EWMA. Используется
детерминированный empty streak:

1. Новый eligible chat начинает с C (15 минут).
2. Poll с новым сообщением переводит чат в A (2 минуты).
3. Активный A после 3 пустых polls переходит в B (5 минут).
4. A/B после 10 пустых polls переходит в C (15 минут).
5. После 30 пустых polls чат переходит в D (60 минут).
6. После 100 пустых polls чат переходит в E (6 часов).
7. Любое новое сообщение снова переводит чат в A.
8. Invalid/private/username errors получают возрастающий retry:
   1ч → 6ч → 24ч → 7д → quarantine.
9. Успешный poll сбрасывает transient error streak.
10. Full batch переводит чат в A и создаёт data-quality alert.
11. Chat с несколькими подписчиками не опрашивается несколько раз.
12. Пустой poll никогда не повышает частоту: новый C не переходит в B/A без
    нового сообщения.
...
- `THROTTLED 75%`: forecast >70%;
- `THROTTLED 50%`: forecast >85%;
- обязательная пауза 5–10 минут после 45 минут непрерывных RPC;
- emergency stop: forecast >95%, любой FloodWait или потеря state storage.
  Для forecast >95% используется `THROTTLED` с мощностью 0% до следующего UTC
  дня, затем только 50% → 75% → 100% после стабильных окон.
...
1. Adaptive interval по empty streak.
...
8. Read-only аудит background RPC; изменение режима updates не входит в v1.
...
4. Сформировать отдельную рекомендацию с измеренной экономией и рисками.

Отключение или использование updates не входит в первую реализацию governor.
Решение не принимается предположением и не выкатывается вместе с этой задачей.
...
- budget gauge;
- RPC 1ч/24ч;
- число назначенных чатов;
- последний FloodWait;
- cooldown/recovery countdown;

График первой версии один: RPC/minute по аккаунтам с safe line. Остальные
значения выводятся числами в fleet summary и карточках.
...
7. Проверить adaptive scheduler в dev/staging на полном наборе классов.
8. Включить adaptive scheduler одним флагом, убедившись, что старый путь
   отключён в том же процессе.
9. Проверить dashboard/alerts.
...
- прямо запретить новые миграции и отдельный сервис в первой версии;
- включить API, компактный UI, alerts, tests и rollout;
