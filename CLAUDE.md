...
Статус: **прод ПЕРЕКЛЮЧЁН на код аудита 12.07 (окно 07:50–09:00 MSK): `audit/fable-fixes` смержена в `main` (ff), тег `audit-fixes-live`, миграция lead_direction01 применена, Redis на AOF+томе (ключи перенесены). Верификация: LLM активен, уведомления доставляются, FloodWait 0. Активный план — `fable_core_plan.md`: 0.1 [x]; далее 0.2 (чистка precision-метрики), фаза A; baseline v2 (задача 0.3) — снять ~19–22.07. ОТЛОЖЕНО: живой тест сценария Б (канал my_leadalert_test_xxx в watched, keyword «фотограф» загружен), Sentry (напомнить владельцу), TLS админки. ⚠️ Владелец не может добавлять каналы через бота: 85 bulk-каналов (insert_sofi_groups) съели лимит 85/60 — см. «Найдено попутно» в fable_core_plan.md.**
