...
Статус: **P0 надёжность (код готов, прод НЕ задеплоен):** очередь уведомлений `BLMOVE` claim/ack/reclaim + digest без потери при mid-flush; идемпотентная оплата Stars/CryptoBot (`provider_charge_id` UNIQUE, миграция `pay_idempotency01`, валидация pre-checkout). 42 related tests passed. Деплой: worker stop → pg_dump → alembic → restart — только по команде владельца. Ранее: Cursor `.cursor/` rules/skills; CI+Deploy workflows; секреты `DEPLOY_*` ещё нужны.
