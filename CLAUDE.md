...
Статус: **🚀 Прод на `main`. Добавлены GitHub Actions: `CI` (pytest + Postgres/Redis) и `Deploy` (SSH после успешного CI на main → `scripts/deploy.sh`: stop worker → alembic → build → up --no-deps). Секреты `DEPLOY_*` ещё нужно завести в GitHub; первый автодеплой — только после проверки владельцем.**
