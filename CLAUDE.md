...
- **Session log (обязательно):** после КАЖДОЙ задачи — дописать краткую запись в конец `docs/SESSION_LOG.md` в формате: `**DD.MM.YYYY HH:MM — Что сделано.** Результат. Ошибки/уроки.` — и обновить краткий статус в `CLAUDE.md §8`. Это обеспечивает контекст для следующей сессии. Без этой записи задача не считается завершённой.
...
### Тарифы (v2, решение #81 — 13.07.2026)
...
| География | 1 страна или ≤3 городов | 1 страна или ≤3 городов | до 5 стран, города без лимита | весь мир (кап: 60) |
| Свои каналы | 1 | 1 | 10 | без лимита (кап: 60) |
| Ключевых слов | 1 | 10 | 50 | без лимита (кап: 60) |
...
Скидки за период: 3 мес −10%, год −20%. Внутренние slug'и: `free`/`start`/`pro`/`business`/`trial`. Лимиты и цены — в `.env`, меняются без правки кода.

### Trial и рефералы

- Trial: 5 дней Business при первом прохождении воронки. +3 дня по реферальной ссылке (итого 8).
- По истечении → Free: контакты скрыты (уведомления без лимита, метрика ценности — широта покрытия).
- End-of-day: 19:00 по часовому поясу, только Free с ненулевой активностью → отчёт «сегодня N заявок, контакты скрыты» с предложением подписки (не лимитный механизм).
...
Статус: **прод на `main` = фазы A+B fable_core_plan + гео-тиеринг поллера (деплой 13.07 06:45 MSK, коммит 087b9be): hot-тир строится по фактической географии подписок (город/страна), зеркалит гео-фильтр _dispatch — hot 205→85, −59% бюджета API; фикс проглоченного BudgetExceeded (батч останавливается). Активный план — fable_core_plan.md: 0.1/0.2/A1-A3/B1/B4-B6 [x]; B2 (few-shot) — следующий заход; B3 ждёт разметки владельца (docs/eval/recall_template_2026-07-12.md); 0.3 baseline v2 ~19–22.07; снапшот латентности ~15.07. ОТЛОЖЕНО: тест сценария Б (канал в Cold-тире, keyword «фотограф» загружен), Sentry, TLS админки, разметка 516 активных каналов без города (общестрановые vs городские), is_ignored для @vietnam_jobs/@danang_movers (ValueError, битые username). НОВОЕ 13.07: тарифы v2 (Старт $9 / Профи $19 / Бизнес $39, отказ от дневного лимита уведомлений, DECISIONS #81) — план `fable_tariff_plan.md`, ветка `feature/tariffs-v2` (Opus). **ФАЗЫ T0–T1 закрыты.** T0: docs+#81, config (тариф start, гео-лимиты), верификация инвентаризации. T1: единая матрица лимитов `_plan_limits` + план start (crud.py); снят дневной лимит уведомлений (sender/cache); гео-лимиты в FSM-воронке (город-на-подписку — реально биндящий; лимит стран подчинён лимиту сегментов при числах v2); start→Paid-формат; **исправлен баг** — активация оплаты не инвалидировала кэш подписок. Сьют 312 passed (4 пред­существующих poller-фейла). T2: три тарифа в платёжном потоке (механика start готова) + экран ошибки оплаты (locales RU+EN, уведомление об истёкшем крипто-инвойсе); **исправлены 2 активных бага крипто-пути** — `_get_user_id` не был определён (NameError), крипто-активация не инвалидировала кэш. T3: экраны и тексты (тон «короче и суше») — карточки 3 тарифов (единый build_plan_screen), живой счётчик меню вместо «0/50», имена планов в счётчиках + гео-строка, триал/апселл, Free-CTA «Открыть контакты — от $9», welcome/about, USERFLOW сверён. T4: воронки продаж — контекстные пейволлы (build_paywall), End-of-day v2 «скрытые контакты», trial-воронка (+фикс: trial_expired не срабатывал), периодика/winback кнопки, годовой апселл, subscription_ending. T5: платные фичи — статистика в боте (menu:stats/​/stats, sent_log+ключ stats:seg), CSV-экспорт Бизнеса (метаданные без текста заявки — решение владельца; миграция sentlog_meta01), digest-режим (миграция user_digest01, буфер+flush-loop). T6 подготовлен (кроме живого деплоя): grandfathering N/A (единственный платящий = владелец 152), черновик анонса, инструментация пейволлов (stats:paywall), **runbook деплоя `docs/runbook_tariffs_v2_deploy.md`**. Теги tariffs-T0…T5-done + T6-prep. Сьют 359 passed (4 пред­существующих poller-фейла). 3 миграции в ветке. **РАЗРАБОТКА ТАРИФОВ v2 ЗАВЕРШЕНА.** Осталось: (1) живой деплой — ВЛАДЕЛЕЦ по runbook (запрещённые при работающем worker команды + правка прод-.env, автономно нельзя); (2) ФАЗА T7 (реактивация платных — запрос владельца). Прод на old-коде. ⚠️ Два блокера для T6.3: (1) прод-.env pro=$7/business=$15 + удалить NOTIFICATIONS_PER_DAY_* одновременно с полями config (extra_forbidden); (2) ⚠️ НАПОМНИТЬ ВЛАДЕЛЬЦУ (его запрос): платные при истечении не даунгрейдятся (доступ бессрочен) — доработать вместе с реактивационной воронкой «сколько заявок пропустил без подписки» → ФАЗА T7 плана (после T6). Прод на old-коде, деплой батчем в T6.**
