...
Фаза: **Фаза 6а (админка CRUD)** ⏳

Сделано: Фазы 0-5 завершены. Конвейер: сообщение→classifier→find_users→Redis queue→sender (throttle 25/с). Free/Paid форматы уведомлений, дедупликация (sha256), лимиты, heartbeat (15 мин). 49 тестов.

Следующее: веб-панель админа (SQLAdmin).
