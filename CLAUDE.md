...
Фаза: **Фаза 4 (userbot + classifier)** ⏳

Сделано: Фазы 0-3 завершены. Кнопки управления словами/каналами, FSM-воронка подписки (сегмент→страна→гео→города→подтверждение→trial), главное меню, настройки/about.

Следующее: userbot + classifier, перехват сообщений.
