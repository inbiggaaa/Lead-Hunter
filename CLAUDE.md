...
Статус: **прод работает с `main` (2 userbot-аккаунта, LLM blocking). Аудит fable_audit.md закрыт целиком: фазы 0/A/B/C/D в ветке `audit/fable-fixes` (офлайн-верификация, сьют 231/3/1), живое переключение — отдельным решением владельца. ⚠️ При переключении: миграцию lead_direction01 накатить ДО старта worker. Следующий план работ — `fable_core_plan.md` (качество ядра: precision 17% → ≥50%, предусловие всех фаз — живое переключение). Каталог: 14 категорий / 69 сегментов, ~1500 keywords. Найдено 12.07: Redis AOF в проде ВЫКЛЮЧЕН (вопреки старому session log), админка на 0.0.0.0:17421 без TLS, SENTRY_DSN пуст — всё включено в задачу 0.1 core-плана.**
